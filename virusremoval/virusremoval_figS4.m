% script virusremoval_figS4.m
%
% Generates Fig. S4 of Pruvôt et al. (2026) using virusremoval_data.mat

%% Load data
load('virusremoval_data.mat')

%% Collect results
% P1: carbon flux network with viruses
% P1a: carbon flux network without viruses constructed from dynamical system model
% P2: carbon flux network without viruses constructed from flux network model

nSim = length(flxs);
recs(nSim) = struct('P1',[],'s1',[],'P2',[],'s2',[]);

for c1=1:nSim
    pn = psel(c1);
    P1 = flxs(c1).with; 
    P1 = diag(1./diag(P1))*P1;
    s1 = [100 0 0 0 0 ]/P1;
    P1a = flxs(c1).wout; 
    P1a(5,4:5) = [-1 1];
    P1a = diag(1./diag(P1a))*P1a;
    s1a = [100 0 0 0 0 ]/P1a;
    P2 = P1;
    %P2(1,3:4) = P2(1,3:4)/(1+P2(1,5));
    P2(1,3) = P2(1,3)+P2(1,5);
    P2(1,5) = 0;
    %P2(2,3:4) = P2(2,3:4)/(1+P2(2,5));
    P2(2,3) = P2(2,3)+P2(2,5);
    P2(2,5) = 0;
    s2 = [100 0 0 0 0 ]/P2;
...
set(gca,'box','on')
title('Through-flow f_B^{tot}','FontWeight','normal', 'FontSize', 12)
ylabel('Flux in FN')
...
set(gca,'box','on')
title('Through-flow f_G^{tot}','FontWeight','normal', 'FontSize', 12)
...
set(gca,'xtick',0:.4:1.2,'ytick',0:.4:1.2)
set(gca,'box','on')
title('Through-flow f_D^{tot}','FontWeight','normal', 'FontSize', 12)
...
set(gca,'box','on')
title('Flux partitioning a_{PG}','FontWeight','normal', 'FontSize', 12)
ylabel('Flux in FN')
...
set(gca,'xtick',0:.2:1,'ytick',0:.2:1)
set(gca,'box','on')
title('Flux partitioning a_{BG}','FontWeight','normal', 'FontSize', 12)
...
set(gca,'xtick',0:.2:1,'ytick',0:.2:1)
set(gca,'box','on')
title('Flux partitioning a_{PD}','FontWeight','normal', 'FontSize', 12)
xlabel('Flux in DS')
ylabel('Flux in FN')
...
set(gca,'xtick',0:.2:1,'ytick',0:.2:1)
set(gca,'box','on')
title('Flux partitioning a_{BD}','FontWeight','normal', 'FontSize', 12)
xlabel('Flux in DS')
...
set(gca,'xtick',0:.2:1,'ytick',0:.2:1)
set(gca,'box','on')
title('Flux partitioning a_{GD}','FontWeight','normal', 'FontSize', 12)
xlabel('Flux in DS')
