function [dx,ph] = model_WEI_rhs(x,p)
% function [dx,ph] = model_WEI_rhs(x,p)
% 
% Evaluates the right-hand side of the dynamical equations of
% the marine microbial ecosystem model from Weitz et al. (2015)
%
% Inputs:
% . x : system state (vector of length 9)
% . p : parameters (structure with 37 fields)
%
% Outputs: 
% . dx : right-hand side of dynamical equations (vector of length 9)
% . ph : carbon flux matrix (matrix of dimensions 10x10)
%
% Notes:
% . The order of the variables in the state vector is
%   H, C, E, Z, VH, VC, VE, xo, xi
% . The flux matrix has an additional row/column 10,
%   corresponding to virus-driven DOC production

% Define meaningful dynamical variables
...
% Convert abundance fluxes to carbon fluxes
ph = diag([p.qH p.qC p.qE p.qZ p.qV p.qV p.qV 1 1])*ph;

% Add row/column corresponding to virus-driven DOC production
aux = -sum(ph(:,end));
ph(10,[8 10]) = [-aux aux];
